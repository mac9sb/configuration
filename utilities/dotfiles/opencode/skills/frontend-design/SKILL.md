...
## AxiomWeb workflow (local environment)

Use the AxiomWeb repo at `~/Developer/tooling/axiom` when a project or request fits the Swift-first stack.

- Follow `~/Developer/tooling/axiom/AGENTS.md` for semantic DSL, typed layout, and color/token usage. Prefer `Section`, `Text`, `.flex(...)`, `.grid(...)`, `.font(...)`, and palette-backed `ColorToken` values over raw tags, styles, or ad-hoc colors.
- Build or preview with the bundled site package:
  - `swift build --package-path site`
  - `swift run --package-path site AxiomHomeSite`
  - Output lands in `site/.output/`; surface changes there in summaries.
- When scaffolding new UI, favor Axiom modules (`AxiomWebUI`, `AxiomWebStyle`, `AxiomWebUIComponents`, `AxiomWebRuntime`). Add typed APIs if you need escape hatches.
- Keep code in Swift targets under `Sources/` or `site/` rather than dropping raw HTML/CSS files. Annotate where new files belong.
