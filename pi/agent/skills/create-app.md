...
- **`@mac9sb/deno-foundation`** (JSR) — auth, KV, routing, logging, static files, Stripe, Sign in
  with Apple
- **`~/Developer/deno-template`** — canonical template to copy from
- **Key pattern**: `mountClientAssets(router)` serves `/i18n.js` and `/webauthn.js` from foundation
  (no copies in `public/`); `mountAuthRoutes(router, kv, opts)` handles all `/auth/*` and
  `/api/session` routes; `createStaticHandler` with `tryFile` handles MIME serving and falls through
  to the router for foundation assets; `mountStripeRoutes` adds billing routes — the app only adds
  domain-specific routes
- **Helpers**: `jsonResponse(data, status?)` and `requireSession(session)` are exported from
  foundation — no need to define them locally in `index.ts`
- **Redirects**: page redirects use `new URL("/path", req.url).href` not `${BASE_URL}/path`.
  `BASE_URL` is only needed for external URLs (magic links, Stripe webhooks, invite links)
